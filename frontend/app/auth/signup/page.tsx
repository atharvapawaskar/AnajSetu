...
import { Recycle } from "lucide-react"; // add this to existing imports
...
const roles = [
    ...
    {
        key: "COMPOST_CENTER",
        label: "Compost Center",
        sub: "Waste Processing",
        icon: Recycle,
        activeStyle: "border-emerald-400 bg-emerald-50",
        activeLabelStyle: "text-emerald-700",
        inactiveStyle: "border-gray-200 bg-white hover:border-emerald-200 hover:bg-emerald-50/40",
        dot: "bg-emerald-500",
        iconBg: "bg-emerald-100",
        iconColor: "text-emerald-600",
        dashboard: "/dashboard/compost",
        perks: ["Receive redirected food", "Track biogas output", "Zero waste impact"],
    },
];
...
export default function SignupPage() {
    ...
    // ── COMPOST CENTER specific ────────────────────────────────────────────────────
    const [compostName, setCompostName] = useState("");
    const [compostAddress, setCompostAddress] = useState("");
    ...
    if (selectedRole === "COMPOST_CENTER") return `${compostName} | ${compostAddress}`;
    return "";
    }
    ...
    if (selectedRole === "DELIVERY") return deliveryArea;
    if (selectedRole === "COMPOST_CENTER") return compostName;
    return "";
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (selectedRole === "COMPOST_CENTER") {
            if (!compostName.trim()) { setError("Please enter your compost center name."); return; }
            if (!compostAddress.trim()) { setError("Please enter your center address."); return; }
        }
                ...
                <div className="bg-white rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 overflow-hidden">
                    ...
                    <div className="px-8 py-7 space-y-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {/* ── COMPOST CENTER FIELDS ─────────────────── */}
                            {selectedRole === "COMPOST_CENTER" && (
                                <>
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="text"
                                                    value={compostName}
                                                    onChange={(e) => setCompostName(e.target.value)}
                                                    placeholder="e.g. Green Earth Compost Center"
                                                    className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                                        ...
                                        <div className="relative">
                                            <MapPin size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                                            <input type="text"
                                                value={compostAddress}
                                                onChange={(e) => setCompostAddress(e.target.value)}
                                                placeholder="e.g. Nashik Industrial Area, Maharashtra"
                                                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition" />
                                        </div>
                                 </div>
                                </>
