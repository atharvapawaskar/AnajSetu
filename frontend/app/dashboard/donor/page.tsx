...
import {
  ...
  ArrowUpDown, SortAsc, Star, MessageSquare, ThumbsUp, Wallet,
} from "lucide-react";
      ...
      <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[72vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" value={foodName} onChange={(e) => setFoodName(e.target.value)} placeholder="e.g. Dal Rice, Biryani, Roti Sabzi"
          ...
          <div>
            ...
            <input type="text" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="e.g. 50 meals, 10 kg"
              className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 transition" />
          </div>
          <div>
...
// ✅ DonationModal Component (Razorpay)
function DonationModal({ donorId, onClose }: { donorId: number; onClose: () => void }) {
  const [amount, setAmount] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState(false);

  const platformFee = amount ? (parseFloat(amount) * 0.02).toFixed(2) : "0.00";
  const donationAmount = amount ? (parseFloat(amount) * 0.98).toFixed(2) : "0.00";
  ...
  async function handleDonate() {
    ...
    try {
      const orderRes = await fetch("http://localhost:8081/api/payments/create-order", {
        method: "POST",
      ...
      await fetch("http://localhost:8081/api/payments/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: donorId,
          ...
          netAmount: parseFloat(donationAmount),
          currency: "INR",
          status: "PENDING",
      ...
      const configRes = await fetch("http://localhost:8081/api/payments/config");
      ...
      const options = {
        ...
        handler: async function (response: {
          ...
          await fetch("http://localhost:8081/api/payments/confirm", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-7 py-5 flex items-center justify-between">
          <div><h2 className="text-white font-extrabold text-xl">Donate Money</h2><p className="text-orange-100 text-xs mt-0.5">Support food rescue across India</p></div>
          <button onClick={onClose} className="w-8 h-8 bg-white/20 hover:bg-white/30 rounded-xl flex items-center justify-center"><X size={16} className="text-white" /></button>
        </div>
        <div className="px-7 py-6 space-y-5">
          {success ? (
            <div className="flex flex-col items-center py-6 text-center">
              ...
              <button onClick={onClose} className="mt-4 px-6 py-2 bg-orange-500 text-white rounded-xl text-sm font-bold">Done</button>
            </div>
          ) : (
            <>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" min="1" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Enter amount"
                    className="w-full pl-8 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 transition" />
                </div>
              </div>
              <div className="flex gap-2">
                {[100, 500, 1000, 5000].map((amt) => (
                  <button key={amt} onClick={() => setAmount(String(amt))}
                    className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all ${amount === String(amt) ? "bg-orange-500 text-white border-orange-500" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-orange-300"}`}>₹{amt}</button>
                ))}
              </div>
              {amount && parseFloat(amount) > 0 && (
                <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 space-y-2">
                  <div className="flex justify-between text-xs text-gray-600"><span>Your donation</span><span className="font-bold">₹{parseFloat(amount).toFixed(2)}</span></div>
                  <div className="flex justify-between text-xs text-gray-500"><span>Platform fee (2%)</span><span>₹{platformFee}</span></div>
                  <div className="border-t border-orange-200 pt-2 flex justify-between text-xs font-bold text-green-700"><span>Goes to food rescue</span><span>₹{donationAmount}</span></div>
                </div>
              )}
              {error && <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl px-4 py-3"><AlertCircle size={15} /><span>{error}</span></div>}
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
                <button onClick={handleDonate} disabled={loading || !amount} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white text-sm font-bold shadow-md transition-all">
                  {loading ? <><Loader2 size={15} className="animate-spin" />Processing...</> : <>💳 Donate ₹{amount || "0"}</>}
                </button>
              </div>
            </>
          )}
...
export default function DonorDashboard() {
  ...
  const [showDonationModal, setShowDonationModal] = useState(false); // ✅ Added State
  ...
  return (
    <div className="min-h-screen bg-orange-50/40">
      <header className="sticky top-0 bg-white border-b border-gray-100 shadow-sm" style={{ zIndex: 100 }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowDonationModal(true)} className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-50 text-orange-600 border border-orange-100 hover:bg-orange-100 font-bold text-xs transition-all">
              <Wallet size={14} />Support NGO Logistics
            </button>
            ...
            <Link href="/auth/login" className="w-9 h-9 rounded-xl border border-gray-200 bg-white hover:bg-red-50 flex items-center justify-center transition-colors"><LogOut size={15} className="text-gray-400" /></Link>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <div className="relative bg-gradient-to-br from-orange-500 via-orange-500 to-orange-600 rounded-3xl px-8 py-7 overflow-hidden shadow-lg shadow-orange-200">
          ...
          <div className="relative flex items-center justify-between gap-6 flex-wrap">
            ...
            <button onClick={() => setShowDonation(true)}
              className="flex items-center gap-2 bg-white text-orange-600 font-bold text-sm px-5 py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all">
                💳 Donate Money
            </button>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <button onClick={() => setShowSortMenu(!showSortMenu)}
                  ...
                  <ArrowUpDown size={14} className="text-orange-500" /><span className="hidden sm:inline">{currentSortLabel}</span>
                </button>
                {showSortMenu && (
          ...
          <div className="px-7 pt-4 flex items-center justify-between flex-wrap gap-3">
            <div className="flex gap-1.5 flex-wrap">
              {[{ key: "ALL", label: "All", count: totalDonations }, { key: "AVAILABLE", label: "Available", count: available }, { key: "CLAIMED", label: "Claimed", count: claimed }, { key: "DELIVERED", label: "Delivered", count: delivered }].map(({ key, label, count }) => (
                <button key={key} onClick={() => setActiveTab(key as any)}
                  className={`flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-xl transition-all ${activeTab === key ? "bg-orange-500 text-white" : "bg-gray-50 text-gray-500"}`}>
                  {label}<span className="text-[10px] bg-white/25 px-1.5 rounded-full">{count}</span>
                </button>
              ))}
            </div>
            <span className="flex items-center gap-1.5 text-xs text-gray-400 font-medium"><SortAsc size={13} className="text-orange-400" /><span className="text-orange-500 font-bold">{currentSortLabel}</span></span>
          </div>

          <div className="px-7 py-5 space-y-3">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16"><Loader2 className="text-orange-400 animate-spin mb-3" /><p className="text-sm text-gray-400">Loading...</p></div>
              ...
              filteredListings.map((listing) => {
                ...
                return (
                  <div key={listing.id} className="flex items-start gap-4 bg-gray-50 hover:bg-orange-50/50 border border-gray-100 rounded-2xl px-5 py-4 transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={`flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full ${st.style}`}>{st.label}</span>
                      </div>
                      ...
                      {listing.status === "DELIVERED" && (
                        <div className="mt-3">
                          {alreadyFeedback ? (
                            <span className="flex items-center gap-1.5 text-xs font-bold text-orange-600 bg-orange-50 px-3 py-1.5 rounded-xl w-fit"><ThumbsUp size={12} /> Feedback Submitted ✓</span>
                          ) : (
                            <button onClick={() => setFeedbackTarget(listing)} className="flex items-center gap-1.5 text-xs font-bold text-amber-600 bg-amber-50 hover:bg-amber-100 px-3 py-1.5 rounded-xl transition-colors"><Star size={12} className="fill-amber-400" /> Rate this Delivery</button>
                          )}
                        </div>
                      )}
                    </div>
                    {listing.status === "AVAILABLE" && (
                      <button onClick={() => deleteListing(listing.id)} className="w-8 h-8 rounded-xl bg-red-50 hover:bg-red-100 flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100"><X size={13} className="text-red-500" /></button>
                    )}
                  </div>
                );
              })
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-2xl px-6 py-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-green-600 mt-0.5">You have helped save <span className="font-bold">{delivered * 40}+ meals</span> and reduced <span className="font-bold">{delivered * 12} kg of CO2</span> this month. 🌱</p>
          </div>
        </div>

        {donorId > 0 && <FeedbackSection userId={donorId} accentColor="orange" />}
      </main>
      ...
      {/* ✅ Added Donation Modal Call */}

      {showDonation && (
        <DonationModal donorId={donorId} onClose={() => setShowDonation(false)} />
      )}
      {showDonationModal && <DonationModal donorId={donorId} onClose={() => setShowDonationModal(false)} />}
      
      {feedbackTarget && <FeedbackModal item={feedbackTarget} onSubmit={handleFeedbackSubmit} onClose={() => setFeedbackTarget(null)} />}
    </div>
  );
}