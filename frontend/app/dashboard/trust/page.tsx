...
function CountdownTimer({ pickupDeadline }: { pickupDeadline?: string }) {
  const [secondsLeft, setSecondsLeft] = useState(0);

  useEffect(() => {
    if (!pickupDeadline) return;
    const expiry = new Date(pickupDeadline).getTime();
  ...
  return (
    <span className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-lg ${
      ...
      isUrgent  ? "bg-red-50 text-red-600 border border-red-200" :
                  "bg-amber-50 text-amber-600 border border-amber-200"
    }`}>
      <Timer size={11} />
      {isExpired ? "Expired ⏰" : `${mins}:${secs.toString().padStart(2, "0")} left`}
    </span>
  );
}
                  ...
                  <div key={listing.id} className="flex items-start gap-4 bg-gray-50 hover:bg-green-50/60 border border-gray-100 hover:border-green-200 rounded-2xl px-5 py-5 transition-all duration-150">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-2">
                          <CountdownTimer pickupDeadline={listing.pickupDeadline} />
